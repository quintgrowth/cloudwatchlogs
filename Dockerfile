...
RUN echo "if \$syslogfacility-text == 'local6' and \$programname == 'nginx' then /var/log/nginx-access.log" >> /etc/rsyslog.d/nginx.conf && \
	echo "if \$syslogfacility-text == 'local6' and \$programname == 'nginx' then ~" >> /etc/rsyslog.d/nginx.conf && \
	echo "if \$syslogfacility-text == 'local7' and \$programname == 'nginx' then /var/log/nginx-error.log" >> /etc/rsyslog.d/nginx.conf && \
	echo "if \$syslogfacility-text == 'local7' and \$programname == 'nginx' then ~" >> /etc/rsyslog.d/nginx.conf
